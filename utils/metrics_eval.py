...
class IouEval:
    def __init__(self, n_classes, device, ignore=None):
        ...
        if ignore!=None:
            ...
            # print("[IOU evaluation] Ignoring class(/es): ", self.ignore.tolist())
        else:
            self.ignore =None
            self.include = torch.from_numpy(np.fromiter((n for n in range(self.n_classes)),int)).long()
            # print("[IOU evaluation] Ignoring none classes")


        # print("[IOU evaluation] Including class(/es): ", self.include.tolist())
        self.reset()
    ...
    def getf1(self):
        tp, fp, fn = self.getStats()
        total_tp, total_fp, total_fn =tp[self.include].sum(), fp[self.include].sum(), fn[self.include].sum()
        f1 = 2*total_tp/(2*total_tp+total_fn+total_fp+ 1e-15)
        return f1
