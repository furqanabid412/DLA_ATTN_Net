...
class KNN(nn.Module):
  def __init__(self, knn=5,search=5,sigma=1.0,
               cutoff=1.0, nclasses=17, apply_gauss=True):
    super().__init__()
    # print("kNN post-processing initialized")
    self.knn = knn
    self.search = search
    self.sigma = sigma
    self.cutoff = cutoff
    self.nclasses = nclasses
    self.apply_gauss = apply_gauss
  ...
  def forward(self, proj_range, unproj_range, proj_argmax, px, py):
    ...
    idx_list = py * W + px
    unproj_unfold_k_rang = proj_unfold_k_rang[:, :, idx_list.long()]
    ...
    unproj_unfold_1_argmax = proj_unfold_1_argmax[:, :, idx_list.long()]
